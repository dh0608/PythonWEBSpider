...
class ProxyIpPool:
    ...
    def save(self, ip, port, protocal):
        ...
        if id not in self.id_list:
            try:
                strsql = 'insert into proxyip VALUES(%s,%s,%s,%s)'
    ...
    def get_proxy(self):
        ...
        print('id:', id)
        strsql = 'select * from proxyip where id="' + id + '"'
        self.cur.execute(strsql)
    ...
    def delRecord(self, id):
        ...
        :return:
        '''
        strsql = 'delete from proxyip where idname="' + id + '"'
        self.cur.execute(strsql)
        self.conn.commit()
