                ...
                try:
                    ...
                    for bander in self.num_seat_dic.keys():
                        current_tr = self.driver.find_by_xpath(
                            '//tr[@datatran="' + bander + '"]/preceding-sibling::tr[1]')
                        if current_tr:
                            # 多座次处理
                            ...
                            for seat in seat_split:
                                if seat:
                                    ...
                                    if current_tr.find_by_tag('td')[seat_type_index].text == '--':
                                        ...
                                        sleep(1)
                                    elif current_tr.find_by_tag('td')[seat_type_index].text == '候补':
                                        print(bander + ' 正在候补，继续尝试……')
                                        sleep(1)
                                    else:
                                        # 有票，尝试预订
                                        ...
                                        for p in self.passengers:
                                            ...
                                            if seat_type_value != 0:
                                                self.driver.find_by_xpath(
                                                    "//select[@id='seatType_" + str(
                                                        key_value) + "']/option[@value='" + str(
...
if __name__ == '__main__':
    # nya
    passengers = ['丁豪']  # 用户名
    from_time = '2019-10-13'  # 出发日期
    # %u90D1%u5DDE%2CZZF 郑州 每个城市对应的cookie是固定的
    from_station = '%u90D1%u5DDE%2CZZF'  # 起始站点 - 来自12306 余票查询 请求 - cookie （此处 - 郑州）
    # %u4E0A%u6D77%2CSHH 上海
    to_station = '%u4E0A%u6D77%2CSHH'  # 结束站点 （此处 - 上海）
    number_seat = ['T206:硬座,无座', 'K378:硬座,无座']  # 车次：座位类型
    # 开始抢票
